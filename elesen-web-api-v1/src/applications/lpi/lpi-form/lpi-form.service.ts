...
export class LpiFormService {
   ...
   async createWithFiles(dto: CreateLpiFormDto, files: { [key: string]: Express.Multer.File[] }): Promise<any> {
       ...
       try {
         ...
         const inspection = this.inspectionsRepository.create({
           ...
           hull_type: dto.jenisHull,
           vessel_type: dto.jenisVesel,
           inspection_date: new Date(dto.tarikhPemeriksaan.replace(/(\d{4}-\d{2}-\d{2})\s+(\d{2})\s*:\s*(\d{2})\s*:\s*(\d{2}\.\d+)/, '$1T$2:$3:$4')),
           end_date: dto.end_date ? new Date(dto.end_date) : undefined,
           location: dto.rumahKemudi_hurufKodZon,
           attandane_form: dto.tandaTanganPembantuImg, //KIV
           vessel_picture: dto.veselImg_keseluruhan, // KIV
           owner_inspector_picture: dto.tandatanganPegawaiImg, // KIV
         ...
         const engine = this.enginesRepository.create({
           ...
           turbo: dto.enjinAdaTurbo_semasaDiperiksa ? 1 : 0,
           ...
           engine_has_turbo_additional: dto.enjinAdaTurbo_tambahan ? 1:0,
           turbo_hp_additional: dto.enjinKuasaKuda_tambahan,
           engine_no_additional: dto.enjinNombor_tambahan,
           has_pev_additional: dto.enjinAdaPEV_tambahan ? 1:0,
         ...
         const sailingEquipment = this.sailingEquipmentsRepository.create({
           full_inspection_lpi_id: savedInspection.id,
           shipping_lights_status: dto.wirelessRadio_status === 'Ada' ? 1 : 0,
           shipping_lights_quantity: parseInt(dto.wirelessRadio_kuantiti) || 0,
           shipping_lights_condition: dto.wirelessRadio_keadaan === 'Baik' ? 1 : 0,
  ...
  async createLpiForm(dto: CreateLpiFormDto) {
    ...
    try {
      ...
      const safetyEquipment = this.safetyEquipmentsRepository.create({
        jacket_status: dto.jaketKeselamatan_status === 'Ada' ? 1 : 0,
        jacket_quantity: parseInt(dto.jaketKeselamatan_kuantiti) || 0,
        jacket_condition: dto.jaketKeselamatan_keadaan === 'Baik' ? 1 : 0,
        jacket_picture_path: dto.jaketKeselamatanImg,
        ...
        fire_extinguisher_picture_path: dto.fire_extinguisher_picture_path,
        wireless_radio_status: dto.wirelessRadio_status === 'Ada' ? 1 : 0,
        wireless_radio_quantity: parseInt(dto.wirelessRadio_kuantiti) || 0,
        wireless_radio_condition: dto.wirelessRadio_keadaan === 'Baik' ? 1 : 0,
        safety_raft: dto.rakitKeselamatan_status === 'Ada' ? 1 : 0,
        safety_raft_quantity: parseInt(dto.rakitKeselamatan_kuantiti) || 0,
        safety_raft_condition: dto.rakitKeselamatan_keadaan === 'Baik' ? 1 : 0,
        raft_picture_path: dto.raft_picture_path,
