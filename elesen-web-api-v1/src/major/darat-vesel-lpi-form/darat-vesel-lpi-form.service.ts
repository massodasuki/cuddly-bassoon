...
export class DaratVeselLpiFormService {
  ...
  async createWithFiles(createDto: CreateDaratVeselLpiFormDto, files: { [key: string]: Express.Multer.File[] }): Promise<any> {
    ...
    const entity = this.daratVeselLpiFormRepository.create({
      ...
      vessel_roof: dtoWithPaths.noVesel_diBumbung ? 1 : 0,
      width_marker_nail: dtoWithPaths.pakuPenandaLebar ? 1 : 0,
      is_no_pev: dtoWithPaths.isNoPEV ? 1 : 0,
    ...
    const safetyEquipments = [
      ...
      { name: 'AIS', type: 'Safety Equipment', data: { status: dtoWithPaths.ais_status, keadaan: dtoWithPaths.ais_keadaan, kuantiti: dtoWithPaths.ais_kuantiti } },
    ];
